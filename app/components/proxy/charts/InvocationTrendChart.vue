<template>
  <div class="relative">
    <div v-if="isEmpty" class="absolute inset-0 flex flex-col items-center justify-center z-10 bg-card/80 rounded-lg">
      <Icon name="lucide:bar-chart-2" class="h-8 w-8 text-muted-foreground/40 mb-2" />
...
const chartData = computed(() => ({
  ...
  datasets: [
    {
      label: 'Total Calls',
      data: isEmpty.value ? generateEmptyData() : sorted.value.map(s => s.total_calls),
      borderColor: 'rgb(44, 221, 109)',
      backgroundColor: 'rgba(44, 221, 109, 0.1)',
      tension: 0.3,
      fill: true,
    },
    {
      label: 'Successful',
      data: isEmpty.value ? generateEmptyData() : sorted.value.map(s => s.success_count),
    ...
    {
      label: 'Errors',
      data: isEmpty.value ? generateEmptyData() : sorted.value.map(s => (s.client_error_count ?? 0) + (s.server_error_count ?? 0)),
