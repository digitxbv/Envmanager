      ...
      <div class="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg border bg-card p-6 shadow-lg">
        ...
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-lg font-semibold">Configure Vercel Sync</h2>
        ...
        <form v-else @submit.prevent="handleSave" class="space-y-6">
          ...
          <TabGroup :selectedIndex="selectedEnvIndex" @change="idx => selectedEnvIndex = idx">
            ...
            <TabPanels>
              <TabPanel v-for="env in environments" :key="env.id" class="space-y-6">
                ...
                <!-- Environment Mapping (only if project selected) -->
                <VercelEnvironmentMapping
                ...
                <div v-if="envConfigs[env.id]!.target" class="border-t mt-4 pt-4">
                  ...
                  <div v-if="showAdvanced[env.id]" class="mt-4">
                    ...
                    <PrefixInput
                      ...
                      @update:modelValue="(val) => updatePrefix(env.id, val)"
                    />
            ...
            <div class="space-y-3">
              ...
              <label class="flex items-center justify-between rounded-lg border p-4 cursor-pointer hover:bg-muted/30">
                <div>
                  ...
                  <p class="text-sm text-muted-foreground">
                    Include secret variables (marked as sensitive in Vercel)
                  </p>
                </div>
                <input
                  type="checkbox"
          ...
          >
            ...
            No environments mapped. Select at least one Vercel target for each environment you want to sync.
          </div>
...
import VercelProjectSelect from './VercelProjectSelect.vue'
import VercelEnvironmentMapping from './VercelEnvironmentMapping.vue'
...
interface EnvConfig {
  target: { projectId: string; teamId: string | null } | null
  environmentMapping: Array<{ envmanager_env: string; vercel_targets: ('production' | 'preview' | 'development')[]; git_branch?: string }>
  prefix: string | null
...
const { projects } = useVercelIntegration()
...
async function handleSave() {
  ...
  try {
    ...
    for (const env of props.environments) {
      ...
      if (envConfig?.target) {
        // Build target_config including Vercel-specific fields AND prefix
        const targetConfig = {
          project_id: envConfig.target.projectId,
          project_name: projects.value.find(p => p.id === envConfig.target?.projectId)?.name || '',
          team_id: envConfig.target.teamId,
    ...
    // Also update platform_sync_configs.target for backwards compatibility with Edge Function
    // Use first configured environment as the "primary" target until Phase 11 updates sync logic
    const firstConfig = configsList[0]
    if (!firstConfig) {
      throw new Error('No environment configuration selected')
    }
    const firstEnvConfig = envConfigs.value[firstConfig.environment_id]

    // Build environment_mapping for all configured environments
    const environmentMapping = configsList.map(c => {
      const envConfig = envConfigs.value[c.environment_id]
      return {
        envmanager_env: c.environment_id,
        vercel_targets: envConfig?.environmentMapping[0]?.vercel_targets || ['production', 'preview', 'development'],
        git_branch: envConfig?.environmentMapping[0]?.git_branch
      }
    })
    ...
    await supabase
      ...
      .update({
        target: {
          // Fields needed by Edge Function (backwards compat)
          project_id: firstEnvConfig?.target?.projectId,
          project_name: projects.value.find(p => p.id === firstEnvConfig?.target?.projectId)?.name || '',
          team_id: firstEnvConfig?.target?.teamId,
          environment_mapping: environmentMapping,
          // New fields for UI
          type: 'per_environment',
          configured_environments: configsList.map(c => {
    ...
    console.error('Failed to save Vercel configuration:', err)
...
watch(() => props.open, async (isOpen) => {
  if (isOpen && props.connection) {
        ...
        if (newConfigs[config.environment_id] && targetConfig?.project_id) {
          newConfigs[config.environment_id] = {
            target: {
              ...
              teamId: targetConfig.team_id || null
            },
    ...
    // TODO: Load teams from validation result (stored on connection or re-fetch)
    teams.value = []
