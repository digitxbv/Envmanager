      ...
      <div class="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg border bg-card p-6 shadow-lg">
        ...
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center gap-3">
            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-[#46E3B7] text-white">
              <Icon name="lucide:server" class="h-5 w-5" />
            </div>
            <div>
              <h2 class="text-lg font-semibold">Configure Render Sync</h2>
        ...
        <form v-else @submit.prevent="handleSave" class="space-y-6">
          ...
          <div v-if="hasAnyConfiguredEnv" class="space-y-4">
            ...
            <div class="space-y-3">
              ...
              <label class="flex items-center justify-between rounded-lg border p-4 cursor-pointer hover:bg-muted/30">
                <div>
                  ...
                  <p class="text-sm text-muted-foreground">
                    Include secret variables (visible in Render dashboard)
                  </p>
                </div>
                <input
                  type="checkbox"
            ...
            <div class="rounded-md bg-yellow-100 p-3 text-sm text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400">
              ...
              Note: Variables synced to Render will be visible in the Render dashboard.
            </div>
          </div>
...
import RenderTargetSelect from './RenderTargetSelect.vue'
import RenderEnvironmentMapping from './RenderEnvironmentMapping.vue'
import PrefixInput from './PrefixInput.vue'
import ServiceScopeSelect from '~/components/services/ServiceScopeSelect.vue'
import {
  useRenderIntegration,
  type RenderSyncTarget
} from '~/composables/useRenderIntegration'
...
const { configureSyncTarget, createEnvGroup, triggerSync } = useRenderIntegration()
...
// =====================================================
// Computed - Owner from connection metadata
// =====================================================

const ownerId = computed(() => {
  const metadata = props.connection?.metadata as Record<string, any> | null
...
async function handleSave() {
  ...
  try {
    ...
    for (const env of props.environments) {
      ...
      let targetId = envConfig.target.id
      let targetName = envConfig.target.name

      // If creating new env group, create it first
      if (envConfig.target.type === 'env_group' && envConfig.target.createNew && envConfig.target.newGroupName) {
        const newGroup = await createEnvGroup(
          props.connection.id,
          ownerId.value,
          envConfig.target.newGroupName
        )
      ...
      // Build target_config including Render-specific fields and prefix
      const targetConfig = {
        owner_id: ownerId.value,
        owner_name: ownerName.value,
        target_type: envConfig.target.type,
        target_id: targetId,
    ...
    $toast.success('Render sync configuration saved')
    emit('saved')
    emit('close')
...
watch(() => props.open, async (isOpen) => {
  if (isOpen && props.connection && props.projectId) {
    ...
    try {
        ...
        for (const config of existingConfigs) {
          ...
          if (targetConfig && configs[config.environment_id]) {
            configs[config.environment_id] = {
              target: {
                type: targetConfig.target_type,
                id: targetConfig.target_id,
                name: targetConfig.target_name,
                createNew: false
              },
      ...
      syncOptions.autoSync = props.existingConfig?.autoSync ?? false
      syncOptions.syncSecrets = props.existingConfig?.syncSecrets ?? true
      syncOptions.syncVariables = props.existingConfig?.syncVariables ?? true
