      ...
      <div class="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg border bg-card p-6 shadow-lg">
        ...
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center gap-3">
            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-[#4285F4] text-white">
              <Icon name="simple-icons:googlecloud" class="h-5 w-5" />
            </div>
            <div>
              <h2 class="text-lg font-semibold">Configure GCP Sync</h2>
        ...
        <!-- Project Info -->
        <div v-if="gcpProjectId" class="mb-6 rounded-md border border-border bg-muted/30 px-3 py-2">
          <p class="text-xs text-muted-foreground">
            <span class="font-medium">GCP Project:</span> {{ gcpProjectId }}
          </p>
        </div>
        ...
        <form v-else @submit.prevent="handleSave" class="space-y-6">
              ...
              <TabPanel v-for="env in environments" :key="env.id" class="space-y-4">
                ...
                <label class="flex items-center justify-between rounded-lg border p-4 cursor-pointer hover:bg-muted/30">
                  <div>
                    ...
                    <p class="text-sm text-muted-foreground">
                      Sync variables from this environment to GCP Secret Manager
                    </p>
                  </div>
                  <input
                    type="checkbox"
          ...
          <div v-if="hasAnyEnabled" class="space-y-4">
            ...
            <div class="space-y-3">
              ...
              <label class="flex items-center justify-between rounded-lg border p-4 cursor-pointer hover:bg-muted/30">
                <div>
                  ...
                  <p class="text-sm text-muted-foreground">
                    Include secret variables (stored as GCP secrets)
                  </p>
                </div>
                <input
                  type="checkbox"
            ...
            <div class="rounded-md bg-yellow-100 p-3 text-sm text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400">
              ...
              All variables are stored as GCP Secret Manager secrets. Secret names are sanitized to comply with GCP naming rules.
            </div>
          </div>
...
const gcpProjectId = computed(() => {
  const metadata = props.connection?.metadata as { project_id?: string } | null
...
async function handleSave() {
  ...
  try {
    ...
    $toast.success('GCP sync configuration saved')
    emit('saved')
    emit('close')

  } catch (err) {
    console.error('Failed to save GCP configuration:', err)
