  ...
  <div class="relative min-h-screen overflow-hidden bg-background px-4 py-12 text-foreground sm:px-6 lg:px-8">
    ...
    <div class="relative mx-auto max-w-4xl rounded-3xl border border-border/60 bg-card/80 p-6 shadow-glow-xl backdrop-blur-xl sm:p-8 lg:p-10">
      ...
      <div v-else class="py-12 text-center">
        <p class="mb-4 text-destructive">Privacy policy not found</p>
        <button
          @click="goBack"
...
const { data: page } = await useAsyncData('privacy', () => {
  return queryCollection('legal').path('/legal/privacy').first()
})
...
useHead({
  title: 'Privacy Policy - EnvManager',
  meta: [
    {
      name: 'description',
      content: 'Privacy Policy for EnvManager - How we collect, use, and protect your data'
    }
  ]
})
</script>
