  ...
  <div class="relative min-h-screen overflow-hidden bg-background px-4 py-12 text-foreground sm:px-6 lg:px-8">
    ...
    <div class="relative mx-auto max-w-4xl rounded-3xl border border-border/60 bg-card/80 p-6 shadow-glow-xl backdrop-blur-xl sm:p-8 lg:p-10">
      ...
      <div v-else class="py-12 text-center">
        <p class="mb-4 text-destructive">Disclaimer not found</p>
        <button
          @click="goBack"
...
const { data: page } = await useAsyncData('disclaimer', () => {
  return queryCollection('legal').path('/legal/disclaimer').first()
})
...
useHead({
  title: 'Disclaimer - EnvManager',
  meta: [
    {
      name: 'description',
      content: 'Disclaimer for EnvManager - Important legal notices and limitations of liability'
    }
  ]
})
</script>
